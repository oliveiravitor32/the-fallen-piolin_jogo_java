...
/*
    Visual das telas de espera: Piolin pulando, "Carregando..." e uma barra
    indeterminada.

    Existe separado das cenas porque o FXGL tem DUAS esperas diferentes, e as duas
    apareciam com visual de engine: a de inicializacao (StartupScene, que mostrava um
    circulo azul girando sobre fundo preto) e a de carregamento de nivel
    (LoadingScene). Montar o visual aqui deixa as duas identicas.
*/
public final class VisualDeCarregamento {

    private static final String CAMINHO_DO_SPRITE = "/assets/textures/walk_piolin1-Sheet.png";
    ...
    public static final Color FUNDO = Color.web("#12101C");

    private VisualDeCarregamento() {
        // Classe utilitaria: nao deve ser instanciada.
    }

    public static StackPane novo(double largura, double altura) {
        Rectangle fundo = new Rectangle(largura, altura, FUNDO);

        VBox coluna = new VBox(26, montarSprite(), montarTexto(), montarBarra());
        coluna.setAlignment(Pos.CENTER);
        coluna.setPrefSize(largura, altura);

        return new StackPane(fundo, coluna);
    }
    ...
    /*
        ...
        O Piolin pula em vez de girar: rotacionar um sprite obriga o JavaFX a
        interpolar as cores e desmancha a grade de pixels, enquanto um deslocamento
        vertical preserva cada pixel intacto.
    */
    private static StackPane montarSprite() {
        /*
            A imagem vem do classpath, e nao de FXGL.texture(): a tela de inicializacao
            e construida antes de o motor registrar o servico de assets, e chamar o
            carregador ali derruba o jogo antes de a janela abrir.
        */
        ImageView piolin = new ImageView(
                new Image(VisualDeCarregamento.class.getResourceAsStream(CAMINHO_DO_SPRITE)));

        // Recorta o primeiro quadro da folha de sprites
        piolin.setViewport(new Rectangle2D(0, 0, LADO_DO_SPRITE, LADO_DO_SPRITE));
    ...
    private static HBox montarTexto() {
    ...
    */
    private static StackPane montarBarra() {
